...
public class BleOperation {
    ...
    //add by Randy for Scan timeout notice
     ...
     * @param context
     */
    private static void initWirelessPen(Context context, String inputMac) {
        mContext = context;
        //String name = getOidName();
        String name = "BLEDongle";

        String pen_mac = "D0:B5:C2:C9:45:DC";
...
//        Toast.makeText(context, name, Toast.LENGTH_LONG).show();
        BleConfig config = new BleConfig.Builder(context).
                ...
                setMatchType(BleConfig.MatchType.MAC).
                //setBluetoothMac(cs_new_device).
                setBluetoothMac(inputMac).
     ...
     * @param context
     */
    public static void openBLEPen(final Context context, String inputMac) {
        if (BleOperation.padNotShield()) {
            BleOperation.initWirelessPen(context,inputMac);
            //startANRSolution();
        }
    }
    ...
    public static BlatandAPICallback wirelessCallback = new SimpleBlatandAPICallback() {
        ...
        public void onNotifyValue(OnChangedBundle onChangedBundle) {
            ...
            EventBus.getDefault().post(new OnNotifyEvent(onChangedBundle));
        }
        ...
        public void onScanTimeOut() {
            //modify by Randy for Scan timeout notice
//            BleLog.e(TAG + "搜索超时");
            BleLog.e(TAG + "完成一个自动扫描周期");
            if(++mScanCount>3){
                EventBus.getDefault().post(new OnNoDeviceFoundEvent());
            }
            super.onScanTimeOut();
        }
        ...
        public boolean onServiceDiscover(BluetoothGatt gatt) {
            BleLog.e(TAG + "蓝牙服务发现");
            EventBus.getDefault().post(new OnFindServiceEvent(gatt));
...
//            EventBus.getDefault().post(new OnReadRSSIEvent(info));
        }
