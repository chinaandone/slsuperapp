...
/**
 * Created by xiong on 2016/2/16.
 */
public class RememberUtil {

    private static final RememberUtil INSTANCE = new RememberUtil();

    private static final String TAG = RememberUtil.class.getSimpleName();
    ...
    private void initWithContext(Context context, String sharedPrefsName) {
        ...
        Log.i(TAG, "RememberUtil took " + delta + " ms to init");
    }
     ...
     */
    public static synchronized RememberUtil init(Context context, String sharedPrefsName) {
        // Defensive checks
        if (context == null || TextUtils.isEmpty(sharedPrefsName)) {
            throw new RuntimeException(
                    "You must provide a valid context and shared prefs name when initializing RememberUtil");
        }
     ...
     */
    private static RememberUtil getInstance() {
        if (!INSTANCE.mWasInitialized) {
            throw new RuntimeException("RememberUtil was not initialized! You must call RememberUtil.init() before using this.");
        }
        return INSTANCE;
    }
     ...
     */
    private <T> RememberUtil saveAsync(final String key, final T value, final Callback callback) {
        // Put it in memory
        mData.put(key, value);
     ...
     */
    public static RememberUtil putFloat(final String key, final float value) {
     ...
     */
    public static RememberUtil putInt(String key, int value) {
        return getInstance().saveAsync(key, value, null);
    }
     ...
     */
    public static RememberUtil putLong(String key, long value) {
        return getInstance().saveAsync(key, value, null);
    }
     ...
     */
    public static RememberUtil putString(String key, String value) {
        return getInstance().saveAsync(key, value, null);
    }
     ...
     * 		and will be passed 'true' if successful, 'false' if not.
     */
    public static RememberUtil putFloat(final String key, final float value, final Callback callback) {
     ...
     * 		and will be passed 'true' if successful, 'false' if not.
     */
    public static RememberUtil putInt(String key, int value, final Callback callback) {
        return getInstance().saveAsync(key, value, callback);
    }
     ...
     * 		and will be passed 'true' if successful, 'false' if not.
     */
    public static RememberUtil putLong(String key, long value, final Callback callback) {
        return getInstance().saveAsync(key, value, callback);
    }
     ...
     * 		and will be passed 'true' if successful, 'false' if not.
     */
    public static RememberUtil putString(String key, String value, final Callback callback) {
        return getInstance().saveAsync(key, value, callback);
    }
     ...
     * 		and will be passed 'true' if successful, 'false' if not.
     */
    public static RememberUtil putBoolean(String key, boolean value, final Callback callback) {
        return getInstance().saveAsync(key, value, callback);
    }
